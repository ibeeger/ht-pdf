...
function getContent(page) {
  ...
  return page.evaluate(function() {
    ...
    var header = getElements(document, 'pdfheader-')
    var footer = getElements(document, 'pdffooter-')

    // Default header and footer e.g. <div id="pageHeader">
    var h = getElement(document, 'pdfheader')
    var f = getElement(document, 'pdffooter')
...
function createSection(section, content, options) {
  ...
  return {
    ...
    contents: phantom.callback(function(pageNum, numPages) {
      var html = c[pageNum];
      pagetotal = numPages;
      var _align = pageNum % 2 == 0 ? "left" : "right";
      if (pageNum === 1 && !html) html = c.first
      if (pageNum === numPages && !html) html = c.last
      return (html || c.default || o.contents || '')
        .replace(/{{align}}/ig, _align)
        .replace(/{{pagenum}}/ig,pageNum)
        .replace(/{{page}}/ig, pageNum+start)
        .replace(/{{pages}}/ig, numPages) + content.styles
    })
  }
}
