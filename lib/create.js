...
function PDF(html, options) {
	...
	if (this.options.script) {
		...
		this.script = path.join(__dirname, '/', 'a4.js')
	}
...
PDF.prototype.getHeight = function(callback) {
	assert(arguments.length > 0, '参数错误');
	this.execHeight(callback);
}

PDF.prototype.execHeight = function exHeight(callback) {
	var child = childprocess.spawn(this.options.phantomPath, [].concat(this.options.phantomArgs, [path.join(__dirname, '/', 'height.js')]));
	var stdout = []
	var stderr = []
